...
export default function GovernanceHub({ token }: { token: string }) {
    ...
    useEffect(() => {
        const fetchGovData = async () => {
            ...
            } catch (e) { toast.error("Governance data unavailable") }
        }
        fetchGovData()
    }, [token])
    ...
    return (
        <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                {[
                    { label: "Active Projects", value: projects.length, icon: Briefcase, color: "text-indigo-500" },
                    { label: "Team Wellness", value: `${stats.avgWellness}%`, icon: Heart, color: "text-rose-500" },
                        ...
                        <CardContent className="p-8">
                            <div className="flex justify-between items-start">
                                <div>
                                    ...
                                    <h4 className="text-4xl font-bold text-slate-800 dark:text-white tracking-tight">{m.value}</h4>
                                </div>
            ...
            <div className="space-y-6">
                <div className="flex justify-between items-center px-4">
                    <h3 className="text-xl font-bold text-slate-800 dark:text-white">Project Oversight</h3>
                    <Button className="bg-indigo-600 hover:bg-indigo-700 text-white rounded-xl text-[10px] font-bold uppercase tracking-widest gap-2 font-brand">
                        New Project
                    </Button>
                </div>
                    ...
                    {projects.map((proj) => (
                        <div key={proj.id} className="bg-white dark:bg-slate-900 p-8 rounded-[3rem] border border-slate-100 dark:border-white/5 shadow-xl flex flex-col justify-between">
                            <div className="flex justify-between items-start mb-6">
                                <div>
                                    <Badge variant="outline" className="h-6 px-3 rounded-lg text-[8px] font-bold border-indigo-500/20 text-indigo-500 uppercase tracking-widest mb-3">Enterprise</Badge>
                                    <h4 className="text-xl font-bold text-slate-800 dark:text-white tracking-tight">{proj.name}</h4>
                                </div>
                                <Badge className="bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300 font-bold uppercase text-[8px] px-3">{proj.status}</Badge>
                            </div>
